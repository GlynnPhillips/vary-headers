...
export default () => {
	...
	document.querySelector('[data-user]').addEventListener('click', (event) => {
		if(event.target.hasAttribute('data-save-session-key')) {
			const key = event.target.previousElementSibling.value;
			model.sessionApiKey(key);
			event.currentTarget.innerHTML = '';
			createUserInput();
		}
	});

	document.querySelector('[data-user]').addEventListener('change', (event) => {
		if(event.target.hasAttribute('data-user-type')) {
			saveProfile();
		}
	});
};
